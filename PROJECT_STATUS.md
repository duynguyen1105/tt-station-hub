...
_Module 5 — Hồ sơ Trạm (Công ty Hóa dầu Trường Thịnh). Last updated: 2026-08-13._
...
- `STORAGE_MOCK=true` — save photos under `public/dev-storage/` for local development.
...
- `anomaly-detection.ts` (5 rules) · `photo-to-reading.ts` ·
...
- **The Barem is read live from Trường Thịnh's spreadsheet and stored nowhere**
  (ADR 0005): each section (c) height is resolved by fetching that Trạm's tab at
  lookup time, so an admin's correction applies to the very next height typed. The
  app has no Barem tables — the two it had are dropped — and no cache, so nothing
  can disagree with the printed table the station holds. An unreadable spreadsheet
  costs manual entry and never blocks the biên bản; `pnpm barem:check` fetches every
  mapped tab, prints the defect report + the `dispensers` comparison, and **writes
  nothing**. Saved receipts keep the litres confirmed at the time.
- Full write-up: [`docs/nhap-hang.md`](docs/nhap-hang.md).

### Integration

- **Tải ảnh** (`/upload` → `app/api/uploads/route.ts` → `lib/photos/upload.ts`):
  admin and kế toán choose Trạm and Ngày, then send Chốt ca, Công nợ or Đo bồn
  photos. Công nợ pairs adjacent photos into one lượt xe; the walk-in option accepts
  meter-only lượt. Photos go to Storage and `ShiftPhoto`, then `lib/photos/ingest.ts`
  and AI; results appear in Cần duyệt or Tồn kho.
...
`debts/visits/[id]/{approve,correct}`, `uploads`, `shifts/[id]/export-misa`, cron routes.
...
- Docs: `docs/local-development.md`, Vietnamese guides (`huong-dan-ke-toan`,
  `huong-dan-nhan-vien-tram`).

### Tests

format, AI extraction + fixtures, anti-truncation, anomaly rules, photo matching,
stock, **shift-sales**, expiry, debt aging, upload timestamp and photo-pair roles,
...
| **Barem spreadsheet** (Trường Thịnh's Google Sheet, no credentials) | ⚠️ **Standing requirement** | The Barem is fetched from this file's CSV export on **every** lookup and stored nowhere (ADR 0005), so it is a **production dependency**: the file must stay readable by **anyone with the link**, at its current id, with its tabs' `gid`s intact (`lib/inventory/barem-sheets.ts`). Tightening the sharing, deleting or re-creating it takes **every Trạm's Barem down at once** — every biên bản then needs its litres typed by hand. Nothing in a Barem needs protecting: it is a calibration table already printed and held at every station. Verify after any edit with `pnpm barem:check`. |
...
| §12.6 | Tank dip → liters conversion (barem bồn)                           | Physical stock entry             | **Half unblocked.** The Barem arrived and is read live from Trường Thịnh's spreadsheet (ADR 0005, no stored copy); the **nhập hàng** side resolves every measured height against it — SL barem and "Nhập vào hầm" are computed per Hầm, and the intake is the tank's own measurement rather than the delivery note's claim. The **tank-dip** side still waits, and waits on **the dip-photo format alone** — not on the Barem, and not on the tables having been dropped: its height comes from an AI reading of a dip-stick photo whose format is unresolved ("01....500", "05....235") — Trường Thịnh will re-photograph. Converting a number that cannot yet be parsed reliably would put a wrong physical stock into the balance, so the dip value stays raw and `lastPhysicalStock` unpopulated. When the format lands, a bulk conversion costs one sheet fetch per Trạm per run, not one per photo. Trường Thịnh also owe an answer on the spreadsheet's own defects (the 1282 mm cliff across 13 tanks) — see `pnpm barem:check`. |
...
- **Deliver cron reminders** over email.
