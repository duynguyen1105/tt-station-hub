  ...
  const updated = await prisma
    .$transaction(
      async (db) => {
        ...
        // Every rule below reads the row as it stands under the ca's lock: a Duyệt or
        // Từ chối that landed since the lookup above is what this one is decided against.
        ...
        if (hasMissingOpening(current)) throw new ReviewRefusal(vi.errors.missingOpening)
        ...
        const blocking = current.anomalyReasons.filter((r) =>
          CONFIRM_REQUIRED_ANOMALIES.includes(r)
        )
        if (blocking.length > 0 && parsed.data.confirm !== true) {
          throw new ReviewRefusal(vi.errors.confirmAnomalies(blocking.map(anomalyLabel)), {
        ...
        if (dispenser) await propagateApprovedClosing(dispenser, current.shiftId, db)
        await writeAudit(
          {
            userId: user.id,
