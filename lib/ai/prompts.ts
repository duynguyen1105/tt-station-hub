...
Montech (red LED): the decimal point is a TINY dot at the BASE of the digits, easily lost in glare or blur — inspect the gap between EVERY pair of digit cells before concluding there is none. On these totalizers the LAST 2 digits (sometimes 3) are usually the decimal part. If you are about to output a 7-8 digit number with no decimal point, look again at the gap before the last 2 digits and record in notes whether a dot is present, absent, or unreadable.
