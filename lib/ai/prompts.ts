...
export const ROUTER_PROMPT = `You are looking at a photo a gas-station attendant sent. Classify it into exactly one type. A photo very often shows BOTH a printed label plate AND the meter — when so, classify by the METER, never as "label_only". The mechanical counter window is often TINY, dark, and sits at the very TOP or BOTTOM edge of the frame (sometimes partially cut off), above or below the big label plate — scan the frame edges carefully before concluding there is no meter; a small strip of white-on-black rolling digits (often next to hand-painted marks like "X3" or "D1") IS a mechanical meter. Ignore any timestamp / GPS watermark the camera app printed on the photo.
