...
/**
 * The paper's product column ("RON 95", "E5 RON 92", "DO 0,05S-V") read as one
 * of the app's fuel types, so a Hầm can be matched to what the tanker brought.
 * The vocabulary is the app's own: the fuel codes the biên bản prompt gives the
 * AI (`lib/ai/prompts.ts`), plus the product names the fuels are seeded under —
 * E0 is "Xăng E5 RON 92" and XANG_A95 is "Xăng RON 95" (`prisma/seed.ts`).
 */
export function fuelTypeFromProductLabel(label: string): string | null {
  ...
  if (/\bE0\b|E5|92/.test(text)) return 'E0'
