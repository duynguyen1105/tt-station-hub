 ...
 *
 * A goods column is a fuel word like any other in the app — "E0", "Dầu DO", a mã
 * hàng on a repainted form — so it is read by the rule that reads a trụ plate and
 * a barem sheet (`resolvePlateFuel`), bound to the Trạm the biên bản is being
 * reviewed at. A column nothing answers for resolves to nothing and the row is
 * simply offered no comparison: a wrong attribution costs money, a missing one
 * costs a glance.
 */
export function deliveryNoteLiters(
  ...
  const matching = products.filter(
    (p) => p.quantityLiters !== null && resolveFuel(p.productLabel) === fuelType
  )
